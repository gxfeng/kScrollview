...
- (void)nextPage
{
    ...
    NSInteger page = (self.collectionView.contentOffset.x + 0.5*(self.collectionView.frame.size.width)) / (self.collectionView.frame.size.width);
    page += 1;
    if (page >= 100*self.imageArray.count) {
        page = 0;
    }
    
    [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:page inSection:0] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:YES];
    /*
    //1 获取当前正在展示的位置
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (index < self.imageArray.count) {
        cell.imageStr = self.imageArray[index];
    }
    return cell;
}
